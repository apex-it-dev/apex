<?php
  ...
  $weekdaynumber = date('N', strtotime("now"));
    ...
    if($num > 0){
      ...
      if(!$qry) { echo __LINE__ . ': ' . $cn->error; goto exitme; /*exit();*/ }
        ...
        if(count($eeAttendance) > 0){
          ...
          if($logs_today){
            if($eeAttendance['loggedin'] != NULL) {
              $logtype = ''; // covid
            } else if($eeAttendance['onleave'] == 0 || 
            ...
            //   $insert_loggeddt_qry = ",'$loggeddt_qry'";
            // }
          }
          if($logs_yesterday){
          ...
          // if($isweekend){
          //   if($logs_today){
          //     $sql_attendance = "INSERT INTO ". ATTENDANCESMST ."(zkdeviceid,zkid,loggedno,startshift$insert_logtype)
          //                     VALUES('$zkdeviceid','$zkid_qry','$loggedno_qry','$startshift'$insert_loggeddt_qry)";
          //     echo $sql_attendance;
          //   } 
          // } else {
          ...
          if(!empty($logtype)){
            $sql_attendance = "UPDATE ". ATTENDANCESMST .
                              " SET $logtype, deviceused = 'biometrics' 
