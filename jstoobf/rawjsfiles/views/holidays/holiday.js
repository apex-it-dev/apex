...
$(function() {
    ...
    $('#btnHolidayPublish').on('click', function() {
      // confirmDialog("This will be published")
      publishWaitBtn();
  
      let wasPublished = false;
      publishHoliday(function(isPublished) {
        // wasPublished = result;
        if (isPublished) {
          publishSuccessBtn();
        } else {
  ...
  function publishSuccessBtn() {
    $('#btnHolidayPublish')
      .html('Successfully published!')
      .removeClass('btn-secondary')
      .addClass('btn-success');
  }
  function publishFailedBtn() {
    $('#btnHolidayPublish')
      .html('Fialed to publish!')
      .removeClass('btn-secondary')
      .addClass('btn-danger');
    setTimeout(() => {
      $('#btnHolidayPublish')
  ...
  function loadHolidayList() {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if ($('#allofc').val() != 1) {
          $('#holidayItemModal #holidaymodal_ofc')
            .attr('disabled', true)
            .on('click', function(e) {
              $(e.target).attr('disabled', true);
            });
          $('#holidayItemModal #holidaymodal_ofc option')
            .filter(() => $(this).text() == $('#ofc').val())
            .prop('selected', true);
        }
  ...
  function genHolidayList(data) {
    if(data == []) return;
    const allHolidayLists = data.holidays.rows;
    ...
    const holidayLists = allHolidayLists.filter(holidayitem => {
      let ofc_exist = false;
      ofcshown.map(ofcitem => {
        if(!ofc_exist) ofc_exist = ofcitem.salesofficeid === holidayitem.office;
      });
      return ofc_exist;
    });
      ...
      $('#holidayListDataTable').DataTable({
        ...
        fnDrawCallback: function() {
          ...
          if ($('#yearFilterHoliday').length == 0 && $('#ofcFilterHoliday').length == 0 && holidayLists != undefined) {
            ...
            $('#yearFilterHoliday, #ofcFilterHoliday, #regionFilterHoliday')
              .off('change')
              .on('change', function() {
            ...
            $('#ofcFilterHoliday').trigger('change');
          }
        },
        columnDefs: [
    ...
    $('#holidayListDataTable tbody').on('click', 'tr', function(e) {
      ...
      holidayLists.find(holiday => {
        if (holiday.id == holidayId) {
          officeId = holiday.office;
          holidayDescFull = holiday.description;
          createdBy = holiday.createdby;
          createdDate = holiday.createddate;
          title = holiday.title;
          isPublished = holiday.published == 1 ? true : false;
        }
      });
      if (isPublished) {
        publishDoneBtn();
      ...
      if(!canupdate) {
        $('#btnHolidayClick').html('');
        $('#btnHolidayClick').attr('onClick', '');
        $('#btnHolidayClick').hide();
        $('#btnHolidayDelete').remove();
        $('#holiday-footer').hide();
      }
      $('#holidayItemModal').modal('show');
    });
  ...
  function publishHoliday(callback) {
    let url = getAPIURL() + 'holidays.php';
    let f = 'publishHoliday';
    ...
    //   .toLowerCase();
    let desc = $('#holidaymodal_desc').val();
    let createdby = $('#holidayUserId').val();
  
    let holidayid = $('#holidayModalId').val();
